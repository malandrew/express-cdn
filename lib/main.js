...
// TODO: Find a way to cache the MD5 hashes so they aren't recomputed on every page load
var renderTag = function(options, assets, attributes) {
  // Set attributes
  attributes = attributes || {};
  // In production mode, check for SSL
  var src = '', position, timestamp = 0, hash = crypto.createHash("md5");
  if (options.production) {
    ...
    if (typeof assets === 'object') {
      ...
      name = _(name).splice(position, 0, '.' + hash.digest("hex"));
      ...
      return createTag(src, _(assets).splice(position, 0, '.' + hash.digest("hex")), attributes) + "\n";
    }
  } else {
...
var processAssets = function(options, results) {
  ...
  for(var i=0; i<results.length; i+=1) {
    var assets = results[i], type = '', fileName = '', position, timestamp = 0, hash = crypto.createHash("md5");
    ...
    if (assets instanceof Array) {
      ...
      for (var k=0; k<assets.length; k+=1) {
        ...
        hash = hash.update(fs.readFileSync(path.join(options.publicDir, assets[k])));
